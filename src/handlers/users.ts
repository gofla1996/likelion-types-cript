...
// `POST /api/users`
export const createUserHander = async (request: Request<{}, {}, RequestUser>, response: Response) => {
  // 클라이언트 요청 정보
...
// `GET /api/users`
export const readAllUsersHandler = async (request: Request, response: Response<User[] | { message: string } | void>) => {
  try {
    const users = await readUsers();
...
// `PATCH /api/users/:id`
export const patchUserHandler = async (request: Request<{ id: string }, {}, RequestUser>, response: Response) => {
